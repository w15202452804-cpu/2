...
- (id)processResponseJSON:(NSData *)jsonData forURL:(NSURL *)url {
    ...
    if ([path containsString:@"qualityAndEffect"]) {
        ...
        if ([list isKindOfClass:[NSArray class]]) {
            NSMutableArray *newList = [list mutableCopy];
            for (int i = 0; i < newList.count; i++) {
                NSMutableDictionary *q2 = newList[i];
                if ([q2 isKindOfClass:[NSMutableDictionary class]]) {
                    q2[@"canPlay"] = @YES; q2[@"needVip"] = @NO; q2[@"free"] = @YES;
                }
            }
            d[@"list"] = newList;
        }
...
- (id)responseObjectForResponse:(NSURLResponse *)response data:(NSData *)data error:(NSError * __autoreleasing *)error {
    ...
    NSString *host = [(NSURL *)response URL].host.lowercaseString;
